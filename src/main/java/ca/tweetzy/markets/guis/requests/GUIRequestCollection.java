...
public class GUIRequestCollection extends Gui {
    ...
    private void draw() {
        ...
        for (int i : Numbers.GUI_BORDER_6_ROWS) {
            setItem(i, GuiUtils.getBorderItem(Settings.GUI_REQUEST_COLLECTION_BORDER_ITEM.getMaterial()));
            if (Settings.GUI_REQUEST_COLLECTION_GLOW_BORDER.getBoolean()) highlightItem(i);
        }

        Markets.newChain().asyncFirst(() -> {
            this.requestedItems = Markets.getInstance().getRequestManager().getRequestsByPlayer(this.player, true).stream().filter(Request::isFulfilled).collect(Collectors.toList());
            return this.requestedItems.stream().skip((page - 1) * 28L).limit(28L).collect(Collectors.toList());
        }).asyncLast((data) -> {
