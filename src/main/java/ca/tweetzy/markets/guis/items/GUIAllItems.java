...
public class GUIAllItems extends Gui {
    ...
    public GUIAllItems(Market market, boolean isEditing) {
        ...
        setDefaultItem(Settings.GUI_ALL_ITEMS_FILL_ITEM.getMaterial().parseItem());
        setRows(6);
    ...
    private void draw() {
        ...
        for (int i : Numbers.GUI_BORDER_6_ROWS) {
            setItem(i, Settings.GUI_ALL_ITEMS_BORDER_ITEM.getMaterial().parseItem());
            if (Settings.GUI_ALL_ITEMS_GLOW_BORDER.getBoolean()) highlightItem(i);
        }

        setPrevPage(5, 3, new TItemBuilder(Objects.requireNonNull(Settings.GUI_BACK_BTN_ITEM.getMaterial().parseMaterial())).setName(Settings.GUI_BACK_BTN_NAME.getString()).setLore(Settings.GUI_BACK_BTN_LORE.getStringList()).toItemStack());
        setButton(5, 4, ConfigItemUtil.build(Settings.GUI_CLOSE_BTN_ITEM.getString(), Settings.GUI_CLOSE_BTN_NAME.getString(), Settings.GUI_CLOSE_BTN_LORE.getStringList(), 1, null), e -> {
