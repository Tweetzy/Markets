...
public final class Titles implements Cloneable {
	...
	public static void sendTitle(@Nonnull Player player,
		...
		if (ReflectionUtils.supports(11)) {
			player.sendTitle(TextUtils.formatText(title), TextUtils.formatText(subtitle), fadeIn, stay, fadeOut);
			return;
		}
		...
		try {
			...
			if (title != null) {
				Object titlePacket = PACKET_PLAY_OUT_TITLE.invoke(TITLE, CHAT_COMPONENT_TEXT.invoke(TextUtils.formatText(title)), fadeIn, stay, fadeOut);
				ReflectionUtils.sendPacket(player, titlePacket);
			}
			if (subtitle != null) {
				Object subtitlePacket = PACKET_PLAY_OUT_TITLE.invoke(SUBTITLE, CHAT_COMPONENT_TEXT.invoke(TextUtils.formatText(subtitle)), fadeIn, stay, fadeOut);
